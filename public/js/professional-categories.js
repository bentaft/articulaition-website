// Professional Categories - Navigation & Routing

document.addEventListener('DOMContentLoaded', function() {
    console.log('🎯 Initializing professional categories...');
...
function handleCategorySelection(type, element) {
    console.log('📍 Navigating to category:', type);
    
    // Add loading state
    element.classList.add('loading');
    
    // Store selection for analytics/tracking
    localStorage.setItem('selected_category', 'business');
    ...
    setTimeout(() => {
        ...
        const analysisPages = {
            'presentation': 'presentation-analysis.html',
            'client-calls': 'client-calls-analysis.html',
            'sales-objections': 'sales-analysis.html',
            'leadership': 'leadership-analysis.html',
            'negotiation': 'negotiation-analysis.html'
        };
        
        const targetPage = analysisPages[type] || 'analysis.html';
        const analysisUrl = `${targetPage}?category=business&type=${type}`;
...
window.businessCategories = {
    ...
    startAnalysis: function(type) {
        const analysisUrl = `upload.html?category=business&type=${type}`;
        window.location.href = analysisUrl;
    },
